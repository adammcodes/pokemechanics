...
/**
 * Script to run a build with monitoring for errors and route failures
...
console.log("Monitoring build output for errors and route failures");
console.log("=".repeat(60));
console.log("");
...
buildProcess.stdout.on("data", (data) => {
  ...
  if (output.includes("Error") || output.includes("Service Unavailable")) {
    const lines = output.split("\n");
    lines.forEach((line) => {
      if (line.includes("Error") || line.includes("Service Unavailable")) {
...
buildProcess.on("close", (code) => {
  ...
  console.log("");
  console.log(
    `Build exit code: ${code} (${buildSucceeded ? "SUCCESS" : "FAILED"})`
  );
  console.log("");
  ...
  try {
    ...
    const pokemonRoutes = Object.keys(prerenderManifest.routes).filter((r) =>
      r.startsWith("/pokemon/")
    );
  ...
  // Report errors
  if (errors.length > 0) {
    console.log(`⚠️  Found ${errors.length} instances of errors in output`);
    console.log("");
    ...
    console.log("✅ No errors detected in build output!");
    console.log("");
  }
