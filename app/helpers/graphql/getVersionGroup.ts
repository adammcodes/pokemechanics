...
// Version groups are static data that rarely changes, so we cache for 7 days
export const getVersionGroup = cache(
  async (gen: string): Promise<VersionGroup> => {
    try {
      const response = await fetchFromGraphQL({
        query,
        variables: { name: gen },
        // Cache this data for 7 days since version groups almost never change
